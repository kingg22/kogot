...
class RuntimeFFIConvenienceGen {
    ...
    private fun buildTransformConvenienceWrapper(iface: Interface): FunSpec? {
        ...
        transformedReturn = transformedReturn ?: iface.returnValue?.type?.let { resolver.resolve(it) }
    ...
    private fun pointerArrayElementType(rawType: String): TypeName = when (rawType) {
        "const GDExtensionConstVariantPtr*" -> resolver.resolve("GDExtensionConstVariantPtr")
        "const GDExtensionConstTypePtr*" -> resolver.resolve("GDExtensionConstTypePtr")
